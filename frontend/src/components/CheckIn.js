...
const CheckIn = ({ onNavigate }) => {
  ...
  const handleManualCheckIn = async () => {
    if (!selectedMember || !manualCode.trim()) {
      alert('Please select a member and enter the verification code');
      return;
    }
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        ...
        {qrCodeError ? (
          ...
          <div className="text-center">
            ...
            <div className="bg-gradient-to-r from-blue-50 to-purple-50 border-2 border-blue-200 rounded-xl p-6 mb-4 max-w-sm mx-auto">
              <div className="text-center">
                <div className="text-sm font-medium text-gray-600 mb-2">6-Digit Access Code</div>
                <div className="text-3xl font-bold text-blue-700 font-mono tracking-wider">
                  {qrCodeData.numeric_code}
                </div>
                <div className="text-xs text-gray-500 mt-2">
                  Members can enter this code manually
                </div>
              </div>
            </div>
            
            <div className="text-sm text-gray-600 space-y-1">
              <p><strong>Instructions:</strong> Members can scan the QR code OR enter the 6-digit code to mark attendance</p>
              <p><strong>Expires:</strong> {new Date(qrCodeData.expires_at).toLocaleTimeString()}</p>
              <p className="text-xs text-gray-500">Both QR code and numeric code refresh automatically every 5 minutes for security</p>
            </div>
          </div>
        ) : null}
      </div>
